...
- Parameter entries follow the typed-argument form: ``name (`type`):``
  followed by an indented description on the next line
...
def calculate_total(items: list[dict], tax_rate: float = 0.0) -> float:
    ...
    Args:
        items (`list[dict]`):
            List of item dictionaries with 'price' keys.
        tax_rate (`float`):
